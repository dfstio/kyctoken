import {
  ...
  RPC_L3_PASSWORD,
  RPC_L3_USER
} from '@kyctoken/config';
const RPC_L3 = {
  url: RPC_L3_ENDPOINT,
  user: RPC_L3_USER,
  password: RPC_L3_PASSWORD
};
const RPC_L3_AXIOS: string =
  'https://' +
  RPC_L3_USER +
  ':' +
  RPC_L3_PASSWORD +
  '@' +
  RPC_L3_ENDPOINT.replace('https://', '');
...
export async function L3(): Promise<number> {
  ...
  const balance = await provider.getBalance(
    '0x1871c9CC2a3A5eB2108f567d87e60131E0EfD32f'
  );
  ...
  if (delay_ms > 10 * 60 * 1000)
    console.log(
      'Blockchain is not producing blocks, last block is',
      block.number,
      'minted',
      delay,
      'ago at',
  ...
  console.log(
    '    pending:',
    parseInt(responseTxPool.data.result.pending.toString())
  );
  console.log(
    '    queued: ',
    parseInt(responseTxPool.data.result.queued.toString())
  );
  ...
  if (response.data.result.inturnPercent !== 100)
    console.log(
      'Clique inturn percent is low: ',
