...
const { CLIENT1_ADDRESS, CLIENT2_ADDRESS, TRADER_ADDRESS } = require('@kyctoken/config');
...
async function main() {
  ...
  const Contract = await hre.ethers.getContractFactory("KERC20");
  ...
  {
  	  const SYM = tokens[i];
	  const ltoken = await Contract.deploy("KYC token "+SYM, "K" + SYM, BigInt(1e9) * BigInt(1e18));
